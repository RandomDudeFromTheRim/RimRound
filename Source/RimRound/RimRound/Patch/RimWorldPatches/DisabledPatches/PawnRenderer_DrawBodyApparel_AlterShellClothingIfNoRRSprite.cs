...
namespace RimRound.Patch
{
    ...
    public class PawnRenderer_DrawBodyApparel_AlterShellClothingIfNoRRSprite
    {
        ...
        static FieldInfo shellRenderedBehindHeadFieldInfo = typeof(ApparelProperties).GetField(nameof(ApparelProperties.shellRenderedBehindHead), BindingFlags.Instance | BindingFlags.Public);
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (shellRenderedBehindHeadFieldInfo is null)
            {
                Log.Error($"shellRenderedBehindHeadFieldInfo was null in {nameof(PawnRenderer_DrawBodyApparel_AlterShellClothingIfNoRRSprite.Transpiler)}");
                return codeInstructions;
            }

            for (int i = 0; i < codeInstructions.Count; ++i)
            {
                if (!(codeInstructions[i].operand is FieldInfo fi) || fi != shellRenderedBehindHeadFieldInfo)
                    continue;
            ...
            if (startJndex != -1)
            {
                ...
                newInstructions.Add(new CodeInstruction(OpCodes.Ldfld, pawnFieldInfo));
        ...
        /// <param name="shellRenderedBehindHead">popped from previous call on stack. Used inside of function rather than adding logical opcodes for simplicity.</param>
        /// <returns><see langword="false"/> to skip the typical offset, <see langword="true"/> to give it one.</returns>
        private static bool ShouldBeDrawnAsShell(bool shellRenderedBehindHead, Pawn pawn) 
        {
            bool isRRBody = pawn is not null && pawn.story is not null && pawn.story.bodyType is not null && BodyTypeUtility.IsRRBody(pawn.story.bodyType);

            if (!shellRenderedBehindHead || isRRBody)
                return false;
